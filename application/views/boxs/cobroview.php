...
function Cobrar_(comprobanteTipo){
  ...
  //Si es una reserva se puede pagar $0
  if(comprobanteTipo != 2 && comprobanteTipo != 0 && comprobanteTipo != 4){
    if(importeAPagar <= 0){
      return false;
    }
  }
  ...
  if(comprobanteTipo == 0 || 
     ...
     comprobanteTipo == 4){
      ...
      if(idOrdenSeleccionada == null) idOrdenSeleccionada = -1;
  ...
  if(comprobanteTipo == 0 || 
     ...
     comprobanteTipo == 4){
      ...
      if(comprobanteTipo == 0 || comprobanteTipo == 1){
        url_ = 'index.php/sale/setSaleMinorista';
      }
      if(comprobanteTipo == 3 || comprobanteTipo == 4){
        url_ = 'index.php/sale/setSaleMayorista';
      }
      $.ajax({
            type: 'POST',
            data: {
        ...
        success: function(result){
                      ...
                      if(comprobanteTipo == 2){ setTimeout("cargarView('sale', 'preventa', '');",300); }
                      if(comprobanteTipo == 3 || comprobanteTipo == 4){ setTimeout("cargarView('sale', 'mayorista', '');",300); }
              },
        error: function(result){
              WaitingClose();
              $('#btnSaveCobroModal').removeAttr("disabled");
    ...
    //Es una reserva, solo controlar que se pague y vincular el pago con la orden 
    ...
    $.ajax({
            ...
            data: {
                    medi:     pagos,
                    des:      desc,
                    oId:      idOrdenSeleccionada,
                    saldo:    importeSaldoAPagar
                  },
        url: 'index.php/sale/setPagoReserva',
        success: function(result){
                      WaitingClose();
                      $('#modalCobro').modal('hide');
                      $('#btnSaveCobroModal').removeAttr("disabled");
                      setTimeout("cargarView('sale', 'listado_reserva', '"+$('#permission').val()+"');",300);
              },
        error: function(result){
              WaitingClose();
              $('#btnSaveCobroModal').removeAttr("disabled");
