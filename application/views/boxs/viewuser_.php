  ...
  if($data['action'] == 'Close'){
   foreach ($data['box']['medios'] as $key => $item):?>
    ...
    <div class="row">
      ...
      <div class="col-xs-5">
        <strong>'.$data['box']['descuentos'].'</strong>
      </div>
    </div><br>';
  }
?>
...
<div class="row">
	...
	<div class="col-xs-5">
      <input type="text" class="form-control" id="cajaImpRendicion" value="<?php echo $data['box']['cajaImpRendicion'];?>" <?php echo ($data['action'] != 'Close' && $data['action'] != 'CloseU' ? 'disabled="disabled"' : '');?>  >
    </div>
</div><br>
