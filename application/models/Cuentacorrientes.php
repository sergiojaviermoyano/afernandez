...
class Cuentacorrientes extends CI_Model
{
	...
	function printRecibo($data = null){
		...
		{
			$this->db->select('cl.*, clientes.cliNombre, clientes.cliApellido, (
				SELECT SUM( cctepDebe ) - SUM( cctepHaber ) 
			...
			$this->db->join('clientes', 'clientes.cliId = cl.cliId');
			...
			$html .= '	<tr style="font-family: Open Sans; font-size: 25px; text-align: center;">
						...
						<tr>
							...
							$html .= 'En concepto de <strong>'.$move['cctepConcepto'].'.</strong><br>';
							if($move['saldo'] > 0)
								$html .= 'Saldo <b>$ '.$move['saldo'].'</b>';
							else
								$html .= 'Saldo <b>$ '.($move['saldo'] * -1).' a favor</b>';
			$html .= 		'</td>
						</tr>';
			$html .= 	'<tr>
							<td><br><br>
